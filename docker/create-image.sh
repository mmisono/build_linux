...
# This script is copied from syzkaller project and modified.
...
if [ -z ${ADD_PACKAGE+x} ]; then
    ADD_PACKAGE="make,sysbench,git,vim,tmux,usbutils,tcpdump,zsh,tig"
fi
...
RELEASE=buster
FEATURE=minimal
SEEK=20479  # image size = 1MB * (SEEK+1)
PERF=false
HOSTNAME="vm"
IMG=/share/vm/${RELEASE}.img
...
ssh-keygen -f /share/vm/$RELEASE.id_rsa -t rsa -N ''
sudo mkdir -p $DIR/root/.ssh/
cat /share/vm/$RELEASE.id_rsa.pub | sudo tee $DIR/root/.ssh/authorized_keys
